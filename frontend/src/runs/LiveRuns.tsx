...
function EvidenceCards({ artifacts }: { artifacts: Record<string, unknown> }) {
  ...
  return <div className="mt-3 grid gap-3 md:grid-cols-2">{Object.entries(labels).map(([key, label]) => {
    ...
    if (!artifact || typeof artifact !== "object" || Array.isArray(artifact)) return <article className="min-w-0 rounded border p-4" key={key}><h3>{label}</h3><p className="text-sm text-slate-600">Not recorded for this run.</p></article>;
