...
async def cleanup() -> None:
    ...
    async with engine.begin() as connection:
        await connection.execute(delete(models.ApprovalDecision.__table__))
        await connection.execute(delete(models.ApprovalRequest.__table__))
        await connection.execute(delete(models.Target.__table__).where(models.Target.name == "r5-target"))
        await connection.execute(delete(models.User.__table__).where(models.User.email.like("r5-%@example.com")))
    yield
    async with engine.begin() as connection:
        await connection.execute(delete(models.ApprovalDecision.__table__))
        await connection.execute(delete(models.ApprovalRequest.__table__))
...
async def create_identity(email: str) -> UUID:
    engine = create_control_plane_engine()
    identity = uuid4()
...
async def create_target(owner: UUID) -> UUID:
    engine = create_control_plane_engine()
    target_id = uuid4()
    now = datetime.now(timezone.utc)
    async with engine.begin() as connection:
...
@pytest.mark.asyncio
async def test_approval_is_visible_after_separate_process_restart_and_four_eyes() -> None:
    requester = await create_identity("r5-requester@example.com")
    approver = await create_identity("r5-approver@example.com")
    target_id = await create_target(requester)
    candidate_id = uuid4()
...
@pytest.mark.asyncio
async def test_expired_approval_cannot_be_enforced() -> None:
    requester = await create_identity("r5-expired@example.com")
    approver = await create_identity("r5-expired-approver@example.com")
    target_id = await create_target(requester)
    engine = create_control_plane_engine()
