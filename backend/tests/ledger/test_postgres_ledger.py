...
@pytest.mark.asyncio
async def test_persisted_chain_survives_restart_and_detects_privileged_tampering(disposable_ledger_database: str) -> None:
    target_id = uuid4()
    engine = create_async_engine(disposable_ledger_database)
    ...
    restarted_engine = create_async_engine(disposable_ledger_database)
